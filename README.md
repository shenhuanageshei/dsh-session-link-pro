...
3. **运行期惰性重试**：`get()` / `update()` 每次发现未挂载就再试一次 `ctx.get("settings")`；成功即挂载并记一行 info（`policy store attached to settings namespace "team-link"`），失败**不重复告警**（一个启动窗口有且仅有一行 warn）。一次性改名迁移（`session-link-pro` → `team-link`）随 attach 执行，不再在 `apply` 当场跑（那时它必然是空操作）。
...
**红线**：任何「未能立刻挂载」都必须留痕，不得存在第二次静默回退。回归锁见 `host-half.test.mjs` 的 **U9**（先 apply、之后 provider 才 active 的时序用例）与 **U11**（settings 彻底缺失时全功能可用且**有且仅有一行** warn）。
...
```
npm test                  # host 476 项 + client 46 项（合计 522 项）
...
断言总数由 `host-half.test.mjs` 自己在结尾打印（`assertion total: 476 (failed: 0)`），本节的计数即取自该行——改测试后请同步本行与 Changelog。
