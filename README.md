...
`webServer` 同一模式（`ctx.inject(["webServer"], …)`），降级语义不变：始终没有 webServer 就只有一行 warn、头部导出按钮不可用，而 `team_link_export` 工具照常工作。
...
**事后链留痕**（评审 round-2 🔵 #3）：attach 之后的两步（内存窗口并入 → 改名迁移）此前是发射后不管、`catch` 也是空的，`migrateLegacyPolicy` 的早退分支（旧命名空间存在但**读不出来**）更是**一行日志都没有**。现在两步各自返回结论标记，attach 结束时统一记一行 info：`post-attach policy chain finished — memory window: <none (no writes while unattached) | not folded (settings namespace already in use) | folded into settings | fold failed>; legacy migration: <no legacy namespace | legacy read failed | no legacy data | migrated | kept (current namespace in use) | failed>`；旧命名空间读失败另按 best-effort ≠ 静默的既有口径补一行 warn。这是**挂载成功之后**的信息行（info 级），与「整个启动窗口有且仅有一行未挂载 warn」的红线互不冲突。

**红线**：任何「未能立刻挂载」都必须留痕，不得存在第二次静默回退。回归锁见 `host-half.test.mjs` 的 **U9**（先 apply、之后 provider 才 active 的时序用例）、**U11**（settings 彻底缺失时全功能可用且**有且仅有一行** warn）与 **F1**（provider 已 active 但 `register()` 恒抛错时，多次 `get`/`update` 之后启动窗口仍**恰一行** warn——两条到达路径共用同一门）；评审 round-2 另加两组：**🟡 #1**（provider 晚挂 → **dispose 其真实 cordis plugin fiber** → 再回归：释放行、未挂载写走内存而非「写入设置失败」、死 scope 未被写入、重挂第二个 provider 后读与写都跟着新命名空间）与 **🔵 #3**（事后链三态：migrated / 无旧数据 / 跳过、每次 attach 的窗口并入结论（含「未并入」），外加「旧命名空间读不出来」的 warn）。
...
断言总数由 `host-half.test.mjs` 自己在结尾打印（`assertion total: 497 (failed: 0)`），本节的计数即取自该行——改测试后请同步本行与 Changelog。497 = 485（0.3.7 交付 + 差异修复轮加固）+ 12（评审 round-2 加固，当次实测：🟡 #1 settings provider 生命周期 8 项——晚挂前置 2、dispose 后的释放行 1、未挂载写走内存 1、死 scope 未被写入 1、provider 回归后重挂 1、读与写跟着新命名空间 2；🔵 #3 事后链留痕 4 项——migrated 路径 1、无旧数据/跳过路径 1、旧命名空间读不出来的 warn 1、每次 attach 的窗口结论（含「未并入」）1）。该轮 9 项在修复前实测全红（`assertion total: 497 (failed: 9)`），还原即全绿；另 3 项是前置/对照断言，修复前即应通过。
