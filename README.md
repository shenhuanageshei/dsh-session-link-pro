...
3. **运行期惰性重试**：`get()` / `update()` 每次发现未挂载就再试一次 `ctx.get("settings")`；成功即挂载并记一行 info（`policy store attached to settings namespace "team-link"`），失败**不重复告警**——「未挂载告警」有**两条到达路径**（激活时未 active / active 但 `register()` 抛错），二者共用同一个一次性门，故**整个启动窗口有且仅有一行 warn**。一次性改名迁移（`session-link-pro` → `team-link`）随 attach 执行，不再在 `apply` 当场跑（那时它必然是空操作）。
...
**红线**：任何「未能立刻挂载」都必须留痕，不得存在第二次静默回退。回归锁见 `host-half.test.mjs` 的 **U9**（先 apply、之后 provider 才 active 的时序用例）、**U11**（settings 彻底缺失时全功能可用且**有且仅有一行** warn）与 **F1**（provider 已 active 但 `register()` 恒抛错时，多次 `get`/`update` 之后启动窗口仍**恰一行** warn——两条到达路径共用同一门）。
...
```
npm test                  # host 485 项 + client 46 项（合计 531 项）
...
断言总数由 `host-half.test.mjs` 自己在结尾打印（`assertion total: 485 (failed: 0)`），本节的计数即取自该行——改测试后请同步本行与 Changelog。485 = 476（0.3.7 原始交付量）+ 9（差异修复轮的加固：F1 回归锁 4 项、旧命名空间失败留痕 2 项、导出文件名共享不变式与路由断言 2 项，另 1 项为 refusing-register fixture 的前置断言）。
