...
- **客户端（防御）**：`readSendCard` 侧再设一道同值上限——因为回执来自**持久化的 `tool/result.meta`**，对核心不透明，可能被手改或异构实现写入；客户端不信任该数组的长度。
...
| U14 | 客户端 A：注册 `key:"team_link_send"` 后渲染为卡片；**无 meta 时回退纯文本**；键写错时回退通用行（不抛错）；**`targets` 行数按 §10.1.2 的 24 行上限有界渲染**（超限在卡内显式标注，标签上的目标总数仍为真，D 面汇总覆盖全量） | §10.1.1 / §10.1.2 |
