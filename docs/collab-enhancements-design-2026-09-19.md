...
>
> **状态**：🚧 **设计完成、待评审、未实施**。本文描述的一切（发送方 A/D 卡片、`/team_session` 自动建队、**自动换届交接**、U13–U24、演练 8/9/10）**都还不在已发布的 0.3.7 里**。
>
...
## 10. 协作增强两项设计（2026-09-19，v1.5）— 🚧 **本次新增，尚未实施**

> **状态**：**设计完成、待评审、未实施**。本节描述的一切（A/D 发送方卡片、`/team_session`、U13–U19、演练 8/9）**都还不在已发布的 0.3.7 里**；已发布部分的说明书在**主文档** `team-upgrade-design-2026-09-17.md` 的 §1–§9（其文首有「文档地图与状态」表）。
...
**体积纪律（具体数值，可断言）**：`message.text` 上限 **2000 码点**（超出则取头 1500 + 尾 400 + 省略标记 3 码点，并置 `truncated: true`；`chars` 记**原始**码点数）；`targets` 上限对齐既有 fan-out **≤8**。理由是 `dsh-tool-fs-search` 的同款做法——**持久化的卡片必须有界**，否则一份长消息会被原样烙进会话日志。这两个数值是**契约**（U13 按它们断言），实施时若要改，改的是本节与 U13，而不是随手在实现里挑一个数。
