...
,
		 ...
		 * (`resolveTargets`), and `buildSendCard` copies every resolved row with no
		 * cap of its own — so a legal broadcast can carry more rows than 8. The
		 * receipt is core-opaque and persisted besides, so a hand-edited log can
		 * carry any number at all. The DISPLAY is therefore bounded here, at the
		 * size of the design's own per-team member cap (§10.2.4 每队成员总数 ≤24:
		 * one full-team broadcast still renders whole), and a receipt past it says
		 * so on the card instead of drawing unbounded rows (round-1 🔵 #2). */
		 ...
		 * target count stays the honest total.
		 */
		 ...
		 * is rendered on exactly ONE of the two faces. */
		function sendCardRowLabel(card, toolName, t) {
			return React.createElement("div", { className: "dshsl-send-rowhead" },
				...
				" · ",
				fillTemplate(t("sendRowTargets"), { count: card.targets.length }));
		}
		 ...
		 *     target count) plus one row per target (target + outcome + detail +
		 *     §3.5 busy) — and nothing else; the rows are capped at
		 *     `SEND_CARD_ROW_LIMIT`, and a receipt past the cap states the
		 *     truncation on A (the face that owns the rows);
		...
		function SendCardView(props) {
			...
			if (detail) {
				// The row count is bounded HERE, and the truncation is stated on the
				// card (§10.1.5 A owns the per-target rows, so the note lives on A and
				// nowhere else): `meta` is core-opaque and persisted, and even a
				// legitimate wildcard broadcast can carry more rows than the fan-out
				// cap suggests — see `SEND_CARD_ROW_LIMIT`.
				...
				return React.createElement("div", { className: "dshsl-relay dshsl-send", "data-slp-send": "row" },
					...
					overflow ? React.createElement("div", { className: "dshsl-send-rows-trunc" }, fillTemplate(t("sendRowsTruncated"), { shown: SEND_CARD_ROW_LIMIT })) : null);
			}
