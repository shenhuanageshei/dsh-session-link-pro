...
,
		...
		function SendCardView(props) {
			...
			if (detail) {
				var rows = card.targets.map(function (target, index) {
		...
		function apply(ctx) {
			...
			// Each of the three §10.1 registrations goes through `guardedSlot` so a
			// refusal costs that one row (差异审计 B3) instead of aborting the
