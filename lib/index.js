// dsh-session-link-pro — node half (host plugin).
//
...
//
// New in -pro:
//  1. session_link_pro_list_sessions — list the sessions of the current
//     workspace (id / title / running state / created time).
//  2. session_link_pro_export — export any session as markdown + JSON into
...
//  3. session_link_pro_send — deliver a message to another live session.
...
const name = "dsh-session-link-pro";
...
const PLUGIN_LABEL = "dsh-session-link-pro";
...
/** Single-line preview of arbitrary text. */
function preview(text, limit = 120) {
	const flat = String(text ?? "").replace(/\s+/gu, " ").trim();
	return flat.length <= limit ? flat : `${flat.slice(0, limit - 1)}…`;
}

/** Truncate long text with an explicit marker (exports must stay honest). */
 ...
 * `session-link-pro`, user-editable through the settings UI and persisted by
...
function createPolicyStore(ctx) {
	...
	if (settings !== undefined && typeof settings.register === "function") {
		try {
			scope = settings.register("session-link-pro", PolicyConfig, { base: structuredClone(DEFAULT_POLICY) });
		} catch (error) {
...
async function exportSession(ctx, sessionId, { format = "both", outputDir } = {}) {
	...
	if (format === "both" || format === "json") {
		...
		await writeFile(file, `${JSON.stringify(buildJsonExport(session, events, title), null, 2)}\n`, "utf8");
		files.push(file);
	}
...
function registerTools(ctx, policy) {
	ctx.effect(() => ctx.tools.register(defineTool({
		name: "session_link_pro_list_sessions",
		...
		async execute(args, exec) {
			try {
				...
				lines.push("");
				lines.push("提示：session_link_pro_export 可导出任意会话（md+JSON）；session_link_pro_send 可发送跨会话消息（需用户批准；接收确认时可选「配对」建立双向免确认通道；目标运行中→注入当前回合，空闲→唤醒为新回合）。");
				return lines.join("\n");
			} catch (error) {
				return `列出会话失败：${describeError(error)}`;
			}
		},
	})), "session-link-pro: list tool");

	ctx.effect(() => ctx.tools.register(defineTool({
		name: "session_link_pro_export",
		...
		async execute(args, exec) {
			...
			try {
				...
				return `已导出会话 ${sessionLabel(sessionId, result.title)}（${result.events.length} 个事件）：\n${result.files.map((file) => `- ${file}`).join("\n")}`;
			} catch (error) {
				return `导出失败：${describeError(error)}`;
			}
		},
	})), "session-link-pro: export tool");

	ctx.effect(() => ctx.tools.register(defineTool({
		name: "session_link_pro_send",
		description: "向另一会话发送跨会话消息（多会话联调用）。发送前需当前用户批准；目标会话运行中→注入当前回合，空闲→唤醒目标并作为新回合处理。已配对的两个会话互发免确认（接收确认时选择「配对：双向免确认」即可建立）。",
		parameters: {
			targetSessionId: { type: "string", required: true, description: "目标会话 id（可用 session_link_pro_list_sessions 查询）" },
		...
		async execute(args, exec) {
			...
			if (target === undefined) return `发送失败：目标会话 ${targetId} 没有活动代理（未在本壳中打开或已退出）。仅支持投递到存活会话。`;
				...
				try {
					const answer = await userQuestions.ask({
						questions: [{
							id: "send-confirm",
							header: "跨会话发送确认",
							question: `发送消息到会话 ${sessionLabel(targetId, undefined)}？\n\n${preview(text, 300)}`,
							options: [
					...
					return `发送失败：发送确认未完成（${describeError(error)}）。`;
				}
				if (choice === "取消") return "已取消：用户拒绝了本次发送。";
				if (choice !== "发送" && choice !== "记住该目标并发送") return "发送失败：发送确认未得到明确同意。";
				if (choice === "记住该目标并发送") {
					try {
						await policy.update({ rememberTargets: [...policy.get().rememberTargets, targetId] });
					} catch {
			...
			if (!paired && !policy.get().trustedSenders.includes(sender.id)) {
				...
				if (view.receiveMode === "reject") return "未投递：目标会话的接收策略为全部拒绝（可在设置 session-link-pro 中调整）。";
				if (view.receiveMode === "ask") {
					...
					try {
						const answer = await userQuestions.ask({
							questions: [{
								id: "receive-confirm",
								header: "收到跨会话消息请求",
								question: `会话 ${sessionLabel(sender.id, senderTitle)} 请求向你发送消息：\n\n${preview(text, 300)}`,
								options: [
						...
						return timedOut
							...
							: `未投递：目标会话用户未能确认（${describeError(error)}）。`;
					} finally {
						clearTimeout(timer);
					...
					if (choice === "拒绝并屏蔽该会话") {
						try {
							await policy.update({ blockedSenders: [...policy.get().blockedSenders, sender.id] });
						} catch {
					...
					if (choice === "配对：双向免确认") {
						try {
							await policy.update({ pairs: [...policy.get().pairs, { a: sender.id, b: targetId, createdAt: Date.now() }] });
						} catch {
							/* best-effort pairing; this send still proceeds */
						}
					} else if (choice === "总是接收该会话") {
						try {
							await policy.update({ trustedSenders: [...policy.get().trustedSenders, sender.id] });
						} catch {
			...
			// ---- deliver -----------------------------------------------------------
			const banner = [
				`📨 [跨会话消息 · 来自会话 ${sessionLabel(sender.id, senderTitle)}]`,
				"",
				text,
				"",
				"（如需回复，可让本会话调用 session_link_pro_send 工具发回）",
			].join("\n");
			const message = {
				id: `slp-${randomUUID()}`,
				role: "user",
				source: { kind: "session-link-pro", plugin: PLUGIN_LABEL, fromSession: sender.id, senderSessionId: sender.id, form: "relay", sentAt: new Date().toISOString() },
			...
			return `已投递到 ${sessionLabel(targetId, targetTitle)}${channelNote}：${running ? "目标正在运行，消息将在步边界注入当前回合" : "目标空闲，已唤醒目标会话并作为新回合处理（消息与回复稍后出现在目标会话中）"}。`;
		},
	})), "session-link-pro: send tool");
}
...
/** Serve `GET /session-link-pro/export?session=<id>&format=md|json` downloads. */
	...
	ctx.effect(() => webServer.register({
		...
		handler: async (req, res) => {
			try {
				...
				const title = await titleOf(ctx, sessionId);
				if (format === "json") {
					const body = JSON.stringify(buildJsonExport(session, events, title), null, 2);
					res.writeHead(200, {
				...
				res.end(`export failed: ${describeError(error)}`);
			}
		},
	}), "session-link-pro: export route");
}
