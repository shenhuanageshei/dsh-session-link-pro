...
function createPolicyStore(ctx) {
	 ...
	 *   (评审 round-2 🔵 #3): "no legacy namespace" (nothing registered for it),
	 *   "legacy read failed", "no legacy data", "migrated", "kept (current
	...
	async function migrateLegacyPolicy() {
		if (legacyScope === null) return "no legacy namespace";
		let legacy;
		try {
	 ...
	 * silently, and never more than once per startup window: the lazy retries
	 * reach this catch again on every `get()` / `update()` while `scope` stays
	 * null, so the line goes through the SAME one-shot gate as the
	 * "not active at activation" line below (U11: the whole startup window
	 * carries exactly one warn, whichever arrival path got there first).
	 *
	 ...
	 */
	function attach(settings, owner, unavailableDetail = null) {
		...
		try {
			...
			if (!attachWarned) {
				attachWarned = true;
				// `unavailableDetail` is the activation-branch arrival (the provider
				// was not active yet / has no register()); otherwise the provider
				// answered but refused this register.
				ctx.logger?.warn?.(`${PLUGIN_LABEL}: settings ${unavailableDetail ?? `register failed (${describeError(error)})`} — 状态仅存进程内存`);
			}
			return;
		}
		try {
	...
	async function adoptMemoryWindow() {
		...
		if (!currentIsDefault) return "not folded (settings namespace already in use)";
		try {
			await update({
