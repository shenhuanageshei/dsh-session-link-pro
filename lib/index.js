...
function pairBetween(view, a, b) {
	return pairRecordBetween(view, a, b) !== null;
}

/** The pair record joining two sessions, or null. The record (not just the
...
function pairRecordBetween(view, a, b) {
	return view.pairs.find((pair) => (pair.a === a && pair.b === b) || (pair.a === b && pair.b === a)) ?? null;
}
...
function createWatchdog(ctx, policy, rotation) {
	...
	async function patrol(options = {}) {
		...
		// background timer for both jobs, and it dies with the plugin. A sweep has
		// no present user, so a receiver whose policy is "ask" is skipped instead of
		// blocking the patrol behind a three-minute dialog.
	...
	async function runRotationSweep(now) {
		...
		try {
			await rotation.sweep({ now, askReceiver: false });
		} catch (error) {
...
function applySetRole(team, request) {
	...
	const record = existing === null
		? roleRecord({ role, current: session, history, pending: null })
		: roleRecord({ ...existing, current: session, history });
...
function registerTeamTools(ctx, policy, rotation) {
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		async execute(args, exec) {
			try {
				...
				// never race this call's own settings write. A sweep has no present
				// user, so receivers whose policy is "ask" are skipped, not prompted.
				const swept = await rotation.sweep({ now: Date.now(), signal: exec.signal, askReceiver: false });
	...
	ctx.effect(() => ctx.tools.register(defineTool({
		...
		description: "读团队黑板（M2，§3.3.3）：一次返回 roster 概要 + decisions.md 末 20 条 + discipline.md 全文 + 各文件 baseHash（decisions 的**仅供参考/审计**——decisions 只追加、不接受 baseHash 参数；discipline 的供整文件替换的乐观锁使用）。任何会话可读；写黑板没有权限门，但读要一次读齐省轮次。文件不存在按空处理并如实标注。黑板根目录 = <team.workspace>/team/<name>/（workspace 在团队首次创建时从会话 agentCwd 捕获）。",
		parameters: {
 ...
 * `options.askReceiver` is false for sweep-originated notices, which have no
 * present user: a receiver whose policy is "ask" is then skipped with an explicit
 * refusal instead of blocking the background sweep for three minutes.
 *
...
async function deliverToTarget(ctx, policy, sender, exec, targetId, text, meta, options = {}) {
	...
	if (target === undefined) return { outcome: "no-agent", text: wellFormed(`发送失败：目标会话 ${targetId} 没有活动代理（未在本壳中打开或已退出）。仅支持投递到存活会话。`) };
	...
	const pair = pairRecordBetween(initialView, sender.id, targetId);
	...
	if (!paired && !policy.get().trustedSenders.includes(sender.id)) {
		...
		if (view.receiveMode === "ask") {
			...
			if (internal && options.askReceiver === false) {
				return { outcome: "refused", text: "未投递：目标会话的接收策略为逐条确认（ask），而插件内部通知不弹确认框（后台清扫没有在场确认方，弹框会阻塞巡逻）。可把发送方加入 trustedSenders 或建立配对后重试。" };
			}
			let timedOut = false;
...
async function fanout(ctx, policy, sender, exec, request) {
	...
	const lines = [`广播 fan-out：${results.length} 个目标${duplicates > 0 ? `（重复目标已去重 ${duplicates} 个）` : ""}`];
...
function settleRotation(team, request) {
	const { role, now, note, migratedPairs, provisional } = request;
	const existing = roleOf(team, role);
...
/** rotation-done: `status` is the ratified/provisional verdict of the claim. */
function doneNotice(team, role, previous, successor, status, now) {
	return `[rotation-done] 团队 ${noticeField(team)} 的角色 ${noticeField(role)} 换届完成（读数 ${readStamp(now)}）：旧任 ${noticeField(previous)} → 新任 ${noticeField(successor)}；信任迁移状态：${status}。${status === "已批准" ? "迁移的 pairs 已是正式通道（旧任持有的 pairs/trustedSenders/rememberTargets 已对称吊销）。" : "迁移的 pairs 是 provisional 通道：24h 内未获批准将自动回退为过门投递，期间经该通道投递的消息不可回收（§3.6.1 诚实声明）。"}`;
}
...
function createRotation(ctx, policy) {
	 ...
	 * exactly as they are for a normal send. `askReceiver: false` is the
	 * sweep-originated mode: a background sweep has no present user to answer a
	 * 3-minute dialog, so a receiver whose policy is "ask" is skipped with an
	 * explicit row instead of blocking the patrol.
	 *
		...
		for (const target of recipients) {
			try {
				const outcome = await deliverToTarget(ctx, policy, { id: senderId }, { signal: options.signal }, target, text, null, {
					internal: true,
					askReceiver: options.askReceiver !== false,
				});
	...
	async function prepare(request) {
		...
		const notice = await broadcastNotice(teamName, caller, freezeNotice(teamName, roleName, caller, successor, now), { askReceiver: true, signal: request.signal });
		return {
			token,
			lines: [
	...
	/** The idempotent replay of a claim whose marker is already on disk (§3.6.2).
	 * The marker and the settlement are written in ONE update and the pending is
	 * cleared in the next, so "pending still present AND marker non-empty" proves
	 * the previous claim did not finish its last step: the roster is settled, the
	 * pairs are migrated, and the only things left are the bookkeeping and — at
	 * worst — the rotation-done notice the team waits on to leave the freeze. The
	 * notice is therefore re-emitted here (at-least-once, never a second
	 * migration); a duplicate notice is noise, a missing one keeps workers frozen. */
	async function replayClaim(request) {
		...
		let cleared = true;
		try {
			await policy.update({ teams: view.teams.map((record) => (record.name === teamName ? clearRotationPending(record, roleName) : record)) });
		} catch (error) {
			cleared = false;
		}
		const status = entry.provisional === null ? "已批准" : "待批准(24h)";
		...
		const previous = (snapshot?.roles ?? []).find((record) => record.role === roleName)?.current ?? "（旧任，见 rotationBackup）";
		const notice = settled
			? await broadcastNotice(teamName, pending.session, doneNotice(teamName, roleName, previous, pending.session, status, now), { askReceiver: true, signal: request.signal })
			: { rows: ["  - （现任已另行变更，不再重复广播 rotation-done）"], summary: "0 投递（换届已被后续变更取代）" };
		const migrated = pending.migratedPairs ?? [];
		return {
			lines: [
				`claim 重放（同一令牌）：团队 ${teamName} 的角色 ${roleName} 的这次换届已由上一次 claim 完成迁移与落定——本次不重复迁移、不重复改动信任数据，只收尾。`,
				`已迁移的通道（${migrated.length} 条，记录来自 pending.migratedPairs）：`,
				...(migrated.length === 0
					? ["  - （无：上次 claim 没有迁移任何 pair）"]
	...
	async function claim(request) {
		...
		if (pending.expiresAt > 0 && pending.expiresAt <= now) {
			...
			// run the cancellation for exactly this role, then refuse.
			const swept = await sweep({ now, signal: request.signal, askReceiver: false, only: { team: teamName, role: roleName } });
			return { error: `claim 失败：令牌已过期（到期 ${readStamp(pending.expiresAt)}，30 分钟有效）。已按过期清扫取消本次换届并广播 rotation-cancelled——旧任 ${entry.current} 仍为 current，冻结解除。${swept.lines.length === 0 ? "" : `\n${swept.lines.join("\n")}`}请由现任重新 prepare。` };
		}
		...
		if (plan.candidates.length === 0) {
			ratified = true;
			chosen = [];
			ratification = "无域内待迁移对：未弹确认框（没有需要批准的迁移），全部与退役者相关的 pairs 随退役清理。";
		} else {
		...
		const settlement = settleRotation(team, { role: roleName, now, note: effectiveNote, migratedPairs: trust.additions, provisional });
		...
		try {
			await policy.update({ teams: teams.map((record) => (record.name === teamName ? clearRotationPending(record, roleName) : record)) });
		} catch (error) {
			bookkeeping = `pending 清除失败（${describeError(error)}）：迁移与落定已生效，pending 里留着 migratedPairs 记录——用同一令牌重放 claim 即完成收尾（不会重复迁移）。`;
		}
		const settledTeam = teams.find((record) => record.name === teamName) ?? settlement.team;
		const mirror = await writeRosterMirror(settledTeam);
		const status = ratified ? "已批准" : "待批准(24h)";
		const notice = await broadcastNotice(teamName, pending.session, doneNotice(teamName, roleName, settlement.previous ?? "（旧任）", pending.session, status, now), { askReceiver: true, signal: request.signal });
		return {
			lines: [
				...
				provisional
					? `provisional 回退窗口：${readStamp(now + ROTATION_PROVISIONAL_TTL_MS)} 到期。到期未批准 → 迁移的 pairs 自动删除、version history 记「${ROTATION_PROVISIONAL_NOTE}」、广播 rotation-expired；${pending.session} 保持 current，信任回退为过门投递（§3.6.2 评审 #5 终态）。补批准：在设置 UI 把该 pair 的 provisional 置 false 即转正式（回退前有效）。`
	 ...
	 * M1 watchdog and touched lazily by every roster read/write. It cancels a
	 * pending nobody claimed inside its 30 minutes (which releases the freeze), and
	 * rolls back provisional trust whose 24h ratification window closed.
	 *
	...
	async function sweep(options = {}) {
		...
		for (const team of teams) {
			...
			for (const entry of team.roles) {
				...
				if (pending !== null && pending !== undefined && pending.expiresAt > 0 && pending.expiresAt <= now) {
					const settled = entry.current === pending.session && Array.isArray(pending.migratedPairs) && pending.migratedPairs.length > 0;
					entry.pending = null;
					if (!settled) cancelled.push({ team: team.name, role: entry.role, incumbent: entry.current, caller: pending.session });
				}
				...
				if (provisional !== null && provisional !== undefined && provisional.expiresAt > 0 && provisional.expiresAt <= now) {
					...
					history.push({ session: provisional.session !== "" ? provisional.session : entry.current ?? "（未知）", from: provisional.at, until: now, note: ROTATION_PROVISIONAL_NOTE });
					entry.history = history;
					entry.provisional = null;
					expired.push({ team: team.name, role: entry.role, successor: provisional.session !== "" ? provisional.session : entry.current });
				}
			}
		}
		if (cancelled.length === 0 && expired.length === 0) return { cancelled, expired, lines: [] };
		const doomed = view.pairs.filter((pair) => pair.provisional === true && pair.expiresAt > 0 && pair.expiresAt <= now);
		const patch = { teams };
		if (doomed.length > 0) {
		...
		try {
			...
			return { cancelled: [], expired: [], lines: [`换届过期清扫写入失败（${describeError(error)}）：本次未生效，下次巡逻/roster 触碰会重试。`] };
		}
		const lines = [];
		for (const item of cancelled) {
			const notice = await broadcastNotice(item.team, item.incumbent, cancelledNotice(item.team, item.role, item.incumbent ?? "（空缺）", item.caller, now), { askReceiver: options.askReceiver !== false, signal: options.signal });
		...
		for (const item of expired) {
			const notice = await broadcastNotice(item.team, item.successor, expiredNotice(item.team, item.role, item.successor ?? "（空缺）", now), { askReceiver: options.askReceiver !== false, signal: options.signal });
		...
		return { cancelled, expired, lines };
	}
