...
//     a top-level summary node from it. The model-visible text is untouched, and
 ...
 * meta"; `v` is the branch point for a future schema change. Every string in
 * it is well-formed at construction: `meta` is validated as JSON and written
 * into the durable session log, where a lone surrogate would kill the next
 ...
 *   as lossless JSON by the Tool registry before it is persisted).
 */
function buildSendCard(request) {
	...
	for (const target of request.targets) {
		...
		const row = {
			sessionId: target.sessionId === undefined ? null : target.sessionId,
		...
		if (target.expr !== undefined && target.expr !== target.sessionId) row.expr = target.expr;
