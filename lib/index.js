...
function doomedProvisionalPairs(pairs, successor, now) {
	return pairs.filter((pair) => pair.provisional === true && pair.expiresAt > 0 && pair.expiresAt <= now && (pair.a === successor || pair.b === successor));
}
...
function createRotation(ctx, policy) {
	...
	async function replayClaim(request) {
		...
		// 评审 #5: an empty marker is a no-candidate rotation, never an approval.
		const status = entry.provisional !== null ? "待批准(24h)" : migrated.length === 0 ? ROTATION_STATUS_NONE : "已批准";
	...
	async function claim(request) {
		...
		// 评审 #5: an empty candidate list opens no dialog, so the verdict is neither
		// "已批准" (nobody was asked) nor "待批准(24h)" (no window was opened).
	 ...
	 *   assert on the decision and not only on its text.
	 */
	async function sweep(options = {}) {
		...
		if (!touched) return { cancelled, expired, closed, lines: [] };
