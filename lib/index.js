...
function createPolicyStore(ctx) {
	 ...
	 */
	function attach(settings, unavailableDetail = null) {
	 ...
	 * `undefined` just means "not (yet) available here". A provider that IS here
	 * but refuses `register` still goes through {@link attach}, which owns the
	 * one-shot warn gate — hence no logging here. */
	...
	if (!attachFrom(ctx)) {
		...
		// register failure uses, so the two arrival paths share one warn.
